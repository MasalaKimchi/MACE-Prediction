...
Provides easy configuration and launching of distributed training jobs.
"""
...
def create_training_command(args):
    """Create the training command with proper arguments."""
    cmd = [
        sys.executable, 'finetune_survival_distributed.py',
    ...
    if args.freeze_epochs > 0:
        cmd.extend(['--freeze_epochs', str(args.freeze_epochs)])
    if args.num_workers:
    ...
    # Add evaluation years
    if args.eval_years:
        cmd.extend(['--eval_years'] + [str(x) for x in args.eval_years])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="🚀 Launch multi-GPU training (auto-detects GPUs)")
    ...
    parser.add_argument('--init', type=str, default='random', choices=['random', 'pretrained'],
                       help='Model initialization')
    parser.add_argument('--pretrained_path', type=str, default='', 
                       help='Path to pretrained model (if init=pretrained)')
    ...
    parser.add_argument('--eval_years', type=float, nargs='+', default=[1, 2, 3, 4, 5],
                       help='Years for time-dependent metrics')
    ...
    print(f"   📊 Detected {gpu_count} GPU(s), using {args.world_size} for training")
